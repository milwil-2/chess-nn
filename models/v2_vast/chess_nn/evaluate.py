...
def evaluate_dataset(model, data_loader):
    """Compute top-1, top-5 policy accuracy and WDL class accuracy on a dataset split."""
    model.eval()
...
def select_move(model, board: chess.Board, board_history: list = None,
                temperature: float = 1.0) -> chess.Move:
    """
    Pick a move using the model's policy output.

    board_history: list of chess.Board, [0] = current, [1..] = prior positions.
    ...
    tensor = torch.from_numpy(boards_to_tensor(board_history)).unsqueeze(0).to(device)
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    print(f"  WDL accuracy:   {metrics['value_acc']:.1f}%")
