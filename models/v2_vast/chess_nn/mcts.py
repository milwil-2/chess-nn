...
class MCTS:
    ...
    def search(self, board: chess.Board, board_history: list = None,
               temperature: float = 1.0, add_noise: bool = False) -> chess.Move:
        """
        ...
        root = Node(prior=1.0)
        self._expand(root, board_history)
        if add_noise:
    ...
    def get_policy(self, board: chess.Board, board_history: list = None,
                   temperature: float = 1.0, add_noise: bool = True) -> dict:
        """
    ...
    def _expand(self, node: Node, board_history: list) -> float:
        """
