...
def cmd_engine(args):
    """
    Run as a chess engine subprocess for head-to-head matches.

    Reads from stdin, writes to stdout:
      position fen <fen>   — set the current position
      go                   — compute and return best move
      quit                 — exit
    ...
    if args.fast:
        ...
        get_move = lambda board: select_move(model, board, temperature=0)
    else:
        from chess_nn.mcts import MCTS
        mcts = MCTS(model, num_simulations=args.sims)
        get_move = lambda board: mcts.search(board, temperature=0)
