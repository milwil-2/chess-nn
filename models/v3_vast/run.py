...
def cmd_supervised(args):
    ...
    MIN_CHUNKS_TO_START = 20

    chunk_paths = sorted(glob.glob(os.path.join(PROCESSED_DATA_DIR, "chunk_*.npz")))
    while len(chunk_paths) < MIN_CHUNKS_TO_START:
        print(f"  Waiting for data... {len(chunk_paths)}/{MIN_CHUNKS_TO_START} chunks ready. "
              f"Checking in 30s. (download_data.py still running)")
        time.sleep(30)
        chunk_paths = sorted(glob.glob(os.path.join(PROCESSED_DATA_DIR, "chunk_*.npz")))
...
def cmd_engine(args):
    """
    Run as a UCI engine subprocess. Speaks the full UCI protocol via
    chess_nn.uci_engine.run(); see that module for command set and known
    limitations (notably `stop` is a no-op because MCTS is synchronous).
    """
    from chess_nn.model import ChessNet
    from chess_nn.utils import load_checkpoint
    from chess_nn.uci_engine import run as uci_run

    model = ChessNet()
    if os.path.exists(args.checkpoint):
        load_checkpoint(args.checkpoint, model)
    else:
        print(f"[engine] checkpoint not found: {args.checkpoint}", file=sys.stderr)
    model.eval()

    uci_run(model, default_sims=args.sims, fast=args.fast)
...
def main():
    ...
    # engine (used by match.py at repo root)
    eng_p = sub.add_parser("engine", help="Run as engine subprocess for match.py")
    eng_p.add_argument("--checkpoint", required=True, help="Path to checkpoint file")
    eng_p.add_argument("--sims", type=int, default=50, help="MCTS simulations per move")
    eng_p.add_argument("--fast", action="store_true", help="Policy-only, no MCTS")
