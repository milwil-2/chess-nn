...
def play_game(mcts: MCTS, max_moves: int = 200) -> list[dict]:
    ...
    board = chess.Board()
    positions = []
    history: deque = deque(maxlen=8)  # board states, most recent first
    resign_counter = 0
    ...
    for move_num in range(max_moves):
        ...
        # Prepend current board to history before this move
        history.appendleft(board.copy(stack=False))
        board_history = list(history)

        temperature = 1.0 if move_num < TEMP_THRESHOLD else 0.1

        print(f"    move {move_num+1:>3}...", end="\r", flush=True)
        visit_dist = mcts.get_policy(board, board_history=board_history, temperature=temperature)
        ...
        with torch.no_grad():
            _, value_logits = mcts.model(tensor)
        v = wdl_to_scalar(value_logits.cpu())  # WDL → scalar ∈ [-1, 1]
        ...
        positions.append({
            "board_tensor": boards_to_tensor(board_history),
    ...
    # Fill in WDL value targets (0=win, 1=draw, 2=loss from current player's POV)
    records = []
    for pos in positions:
        if result == "1-0":
            value_cls = 0 if pos["turn"] == chess.WHITE else 2
        elif result == "0-1":
            value_cls = 2 if pos["turn"] == chess.WHITE else 0
        else:
            value_cls = 1
    ...
    for game_idx in range(num_games):
        ...
        if (game_idx + 1) % chunk_size == 0 or is_last:
            ...
            np.savez_compressed(
                ...
                values=np.array(all_values, dtype=np.int64),
            )
