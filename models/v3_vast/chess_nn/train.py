...
def train(chunk_paths: list[str], resume: bool = False):
    ...
    for epoch in range(start_epoch + 1, NUM_EPOCHS + 1):
        ...
        for boards, policy_targets, value_targets, legal_masks, sf_policies in train_loader:
            boards         = boards.to(DEVICE)
            policy_targets = policy_targets.to(DEVICE)
            value_targets  = value_targets.to(DEVICE)
            legal_masks    = legal_masks.to(DEVICE)
            sf_policies    = sf_policies.to(DEVICE)
            ...
            with torch.autocast(device_type=DEVICE.type, dtype=torch.bfloat16, enabled=USE_BF16):
                ...
                # Auxiliary Stockfish loss — only for annotated positions (sf_policy != -1)
                sf_mask = sf_policies != -1
                if sf_mask.any():
                    sf_loss = F.cross_entropy(
                        masked_logits[sf_mask], sf_policies[sf_mask]
                    )
                else:
                    sf_loss = torch.tensor(0.0, device=DEVICE)

                total_loss = policy_loss + VALUE_LOSS_WEIGHT * value_loss + SF_LOSS_WEIGHT * sf_loss
        ...
        with torch.no_grad():
            for boards, policy_targets, value_targets, legal_masks, _sf in val_loader:
                boards         = boards.to(DEVICE)
                policy_targets = policy_targets.to(DEVICE)
                value_targets  = value_targets.to(DEVICE)
                legal_masks    = legal_masks.to(DEVICE)
