...
from chess_nn.board_encoding import board_to_tensor, boards_to_tensor
...
class TestModel(unittest.TestCase):
    ...
    def test_get_activations_shape(self):
        board = chess.Board()
        t = torch.from_numpy(boards_to_tensor([board])).unsqueeze(0)
    ...
    def test_output_shapes_mps(self):
        ...
        x = torch.randn(4, INPUT_PLANES, 8, 8).to("mps")
        policy, value = model(x)
        _mps_sync()
        self.assertEqual(policy.shape, (4, 4672))
        self.assertEqual(value.shape, (4, 1))
...
class TestTimingDiagnostics(unittest.TestCase):
    ...
    def test_mps_synchronize_cost(self):
        model = ChessNet().to("mps")
        x = torch.randn(BATCH_SIZE, INPUT_PLANES, 8, 8).to("mps")
        _ = model(x)  # warm up
        with Timer() as t:
            torch.mps.synchronize()
    ...
    def test_tensor_to_mps_time(self):
        x = torch.randn(BATCH_SIZE, INPUT_PLANES, 8, 8)
        with Timer() as t:
            x_mps = x.to("mps")
            _mps_sync()
        print(f"\n  [timing] ({BATCH_SIZE}, {INPUT_PLANES}, 8, 8) tensor to MPS: {t.elapsed*1000:.1f}ms")
