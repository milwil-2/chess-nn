...
def make_dataloaders(chunk_paths: list[str]):
    ...
    train_loader = DataLoader(train_set, batch_sampler=train_sampler, num_workers=DATA_WORKERS, pin_memory=True)
    val_loader   = DataLoader(val_set,   batch_sampler=val_sampler,   num_workers=DATA_WORKERS, pin_memory=True)
    test_loader  = DataLoader(test_set,  batch_sampler=test_sampler,  num_workers=DATA_WORKERS, pin_memory=True)
