...
class ChessNet(nn.Module):
    ...
    def __init__(self):
        ...
        # 8 filters (vs 2) preserves more of the body's 128-filter representation
        ...
        # 4 filters (vs 1) gives the head room to encode position nuance
        self.value_conv = nn.Conv2d(NUM_FILTERS, 4, kernel_size=1, bias=False)
        self.value_bn = nn.BatchNorm2d(4)
        self.value_fc1 = nn.Linear(4 * 8 * 8, 256)
        self.value_fc2 = nn.Linear(256, 1)

        # Dropout applied in heads only — 0.1 (vs 0.3): model is large enough that
        # underfitting is the bigger risk
