...
Two losses are combined:
  ...
  - Value loss:  cross-entropy over WDL classes {win=0, draw=1, loss=2}

Total loss = policy_loss + VALUE_LOSS_WEIGHT * value_loss
"""
    ...
    for epoch in range(start_epoch + 1, NUM_EPOCHS + 1):
        ...
        with torch.no_grad():
            for boards, policy_targets, value_targets, legal_masks in val_loader:
                ...
                val_value_loss  += F.cross_entropy(value_pred, value_targets).item()
                ...
                correct_value += (value_pred.argmax(dim=1) == value_targets).sum().item()
        ...
        print(f"  WDL accuracy       : {value_acc:.1f}%  (win/draw/loss class prediction)")
        ...
        # Save best model
        val_total = avg_val_policy + VALUE_LOSS_WEIGHT * avg_val_value  # combined loss for checkpoint selection
